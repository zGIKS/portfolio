...
publishedAt: "2026-06-17T22:45:00Z"
status: "published"
---

# Clean Operating System Architecture Guide for NixOS
...
├── flake.nix
├── flake.lock
├── .sops.yaml            # Tool config for SOPS (must be in root)
├── README.md
├── hosts/                # Host Composition (machines)
│   ├── laptop/           # hosts/laptop/secrets/secrets.yaml (Host-specific secrets)
│   ├── desktop/
│   └── server/
├── home/                 # User Space (Home Manager & user applications)
│   └── <username>/
└── modules/              # System OS Layers (exactly 8 folders)
    ├── hardware/
    ├── kernel/
    ├── filesystem/
    ├── networking/
...
1. Prefer **one flake** per repo with multiple `nixosConfigurations` (one per host).
2. **Pristine Root Rule**: Keep the repository root uncluttered. Group all system-level logic under `modules/` and user-level preferences under `home/`.
3. **Host-Specific Secrets**: To respect Bounded Contexts, machine-specific encrypted secrets (`secrets.yaml`) should live directly inside the host's directory (e.g., `hosts/gaia/secrets/secrets.yaml`), rather than a global top-level `secrets/` folder. Use `sops.defaultSopsFile = ./secrets/secrets.yaml;` locally per host.
